...
app.post('/setTheme',(req,res)=>{
  ...
  if(!username||!theme) return res.status(400).send({error:'Onvolledige data'});
  const accounts = loadJSON(accountsFile);
  const user = accounts.find(u=>u.username===username);
  if(!user) return res.status(400).send({error:'Gebruiker niet gevonden'});
  user.theme = theme;
  saveJSON(accountsFile,accounts);
  res.send({message:'Theme aangepast!'});
});

app.get('/getTheme/:username',(req,res)=>{
  const accounts = loadJSON(accountsFile);
  const user = accounts.find(u=>u.username===req.params.username);
  if(!user) return res.status(400).send({error:'Gebruiker niet gevonden'});
...
io.on('connection',(socket)=>{
  ...
  socket.on('delete message',(id)=>{
    ...
    for(const key in allPrivate){
      const msgs = allPrivate[key];
      const msgPriv = msgs.find(m=>m.id===id);
      if(msgPriv && msgPriv.user===socket.username){
        allPrivate[key] = msgs.filter(m=>m.id!==id);
        saveJSON(privateChatFile,allPrivate);
        const [userA,userB] = key.split('_');
        const sockA = online.get(userA); const sockB = online.get(userB);
