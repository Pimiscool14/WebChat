document.addEventListener('DOMContentLoaded', () => {
  ...
  function formatMessage(text){
    ...
    return text.replace(urlRegex,(url)=>{
      ...
      else if(url.match(/\.(jpg|jpeg|png|gif|webp)$/i)) embed=`<br><img src="${url}" alt="afbeelding">`;
  ...
  // Chat
  chatForm.addEventListener('submit',e=>{e.preventDefault(); const txt=messageInput.value.trim(); if(!txt) return; socket.emit('chat message',{user:username,msg:txt,type:'text'}); messageInput.value='';});
  messageInput.addEventListener('keypress',e=>{if(e.key==='Enter'){e.preventDefault(); chatForm.dispatchEvent(new Event('submit'));}});

  // Photo
  photoInput.addEventListener('change', e=>{const file=e.target.files[0]; if(!file) return; const reader=new FileReader(); reader.onload=()=>socket.emit('chat message',{user:username,msg:reader.result,type:'image'}); reader.readAsDataURL(file);});
  ...
  function addMessageToList(data){
    ...
    if(data.type==='image'){msgSpan.innerHTML=`<img src="${data.msg}" alt="afbeelding">`;}
